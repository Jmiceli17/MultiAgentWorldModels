...
def ds_gen():
    ...
    for j, fname in enumerate(filenames):
        ...
        if done.shape[0] > num_agents*args.max_frames: #TODO: Shouldn't this be always false
            i+=1
            continue
        ...
        n_pad = num_agents* args.max_frames - z.shape[0] # pad so they are all a thousand step long episodes
...
def create_tf_dataset():
    dataset = tf.data.Dataset.from_generator(ds_gen, output_types=(tf.float32, tf.float32, tf.float32, tf.bool), output_shapes=((num_agents*args.max_frames, args.z_size+num_agents), (num_agents*args.max_frames, args.a_width), (num_agents*args.max_frames,), (num_agents*args.max_frames,)))
    return dataset
...
print("djlamfds", mu_dataset.shape, logvar_dataset.shape, r_dataset.shape)
