...
def evaluate_batch(ctrl_list, test_env, test_seed, num_test_episode, max_len=-1):
...
def main(args):
  # global optimizer, num_episode, num_test_episode, eval_steps, num_worker, num_worker_trial, antithetic, seed_start, retrain_mode, cap_time_mode, env_name, exp_name, batch_mode, config_args
    ...
    while gen < 100:
        # Get the current weight estimates from each controller's optimizer and set the controller
        for es, controller in zip(ES_LIST, CONTROLLER_LIST):
            solutions = es.ask()    # Returns a list of solutions that need to be evaluated
            print("[DEBUGGING] SHAPE OF SOLUTIONS: {}".format(np.shape(solutions)))
            weights = solutions[0]  # TODO: need to construct an objective function that we can pass the list of solutions into
            controller.set_model_params(weights)
        ...
        for es, controller in zip(ES_LIST, CONTROLLER_LIST):
            ...
            es.tell(reward_list)
            print("[DEBUGGING] called tell")

            # TODO: Ideally we implement something similar to this:
            # X = es.ask()  # get list of new solutions
            # fit = [cma.ff.rosen(x) for x in X]  # call fct with each solution
            # es.tell(X, fit)  # feed values
            
            es_solution = es.result()
            print("[DEBUGGING] called result")
        ...
        if (gen % eval_steps == 0): # evaluate on actual task at hand
            ...
            # Evaluate using the test_env
            reward_eval_list = evaluate_batch(CONTROLLER_LIST, test_env, num_test_episode, test_seed=gen, max_len=-1)  # must be updated to support multiple controllers
            ...
            # Store the history of best evaluations
            curr_time = int(time.time()) - start_time
            best_record = [gen, curr_time, "improvement", improvement, "curr", reward_eval, "prev", prev_best_reward_eval, "best", best_reward_eval]
            history_best.append(best_record)
            with open(filename_hist_best, 'wt') as out:
                res = json.dump(history_best, out, sort_keys=False, indent=0, separators=(',', ':'))
